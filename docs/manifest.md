...
## `atana.unctad` — UNCTAD Creative Economy ✅ Live (MotherDuck synced; 1 corrupt parquet flagged in `raw/unctad/goods_growth.parquet` — aggregation, per-year tables `goods_value_*` are healthy)
...
| `corporate_canon` | 5,735 | Mapeamento canônico de razões sociais (curado). |
| `cycle_status_map` | 67 | Funil ATTRição aprovação → captação → execução. |
| `propostas_recentes` | ~50k+ | Propostas em estágio pré-PRONAC. Não usado por Análise publicada. |

**Prior manifest state** listed `projetos` at 26,203 rows — that was the count of the smaller `projetos_v2` subset; the true `projetos` table has been at 48,189 since commit `4e48176`. Coverage went from "46 % of universe" (v2 metric) to **~85 % of universe** (canonical `projetos` metric). Published Notes/Análises citing the older figures remain correct at time of writing; new work uses 48,189.

ETL: `etl/salic__bulk_download.py` (paginated fetch + WAF bypass) · `etl/salic__jsonl_to_parquet.py` (flatten). Methodology: `docs/methodology/salic_api.md`.
...
## `atana.oecd_ai` — OECD AI Papers (methodological frame for the Atana AI Exposure Index) ⏳ Tier 1 ✅ live (GitHub `9f8611b` + MotherDuck) · Tier 2 (Phase 5d, 2026-06-26) built locally, pending push

Source: **OECD Artificial Intelligence Papers** series (OECD-OPSI), **No. 59** *The OECD AI exposure measure: Mapping the OECD AI Capability Indicators to occupations* (May 2026, 58 pp) and **No. 60** *Benefits of AI Openness* (3 Jun 2026, 46 pp; G7 discussion paper at the French presidency's request). Phase 5c — methodological-frame source for the Atana AI Exposure Index (Vol. 1) and Vol. 2. **Phase 5d (2026-06-26)** extends the schema with three PDF-extracted occupation-level tables from No. 59 chapter 6 — Path B of the scoping memo (Path A, the JS-rendered oecd.ai per-occupation CSV, remains pending a manual download).
...
| `ai_capability_domains` | 9 | — | The 9 OECD AI capability domains (language, social interaction, problem solving, **creativity ★**, metacognition, knowledge/learning/memory, vision, manipulation, robotic intelligence) with cultural-occupation entry-points |
| `skill_demand_high_exposure` | 6 | 2026 | Demand-side findings from No. 59 — share of vacancies in high-AI-exposure occupations requiring each skill group. **Originality demand rose 25 % → 33 %**, the fastest-rising cognitive sub-skill |
| **`occupations_creativity_distance`** | **10** | — | Table 6.4a Creativity column top-10 — occupations AI is furthest from on creativity. **8 of 10 are SOC group 27 cultural** (Music Directors and Composers, Choreographers, Special Effects Artists, Producers and Directors, Art Directors, Multimedia Artists, Set and Exhibit Designers, Fashion / Interior Designers). The occupation-weighted empirical anchor of the Atana Authenticity Paradox |
| **`exposure_extremes_ranking`** | **20** | — | Table 6.5 top/bottom 10 overall — most-exposed tail dominated by clerical (SOC 43, tied at 0.00 gap); least-exposed tail by chief executives (11.71), medical specialists, firefighters/police, lawyers (9.53) and judges (9.53). **Zero SOC group 27 cultural in either tail** — the structural argument for per-domain decomposition over composite-only reading |
| **`felten_oecd_correlations`** | **10** | — | Table 6.6 per-domain Felten × OECD correlations. **Overall correlation only 0.34. Creativity is the diagnostic outlier at 0.25** — the empirical proof Felten is creativity-blind. Vision / Manipulation / Robotic intelligence carry NEGATIVE correlations (Felten was language-centred) |

⚠️ **Methodological-frame source — NOT a cultural classification.** Central caveats — `docs/methodology/oecd_ai_papers.md` §6 — (1) the papers measure OECD labour markets directly; LATAM not in analytical scope; (2) the OECD's `creativity` domain is functional ("production of novel and valuable outputs") and is *not* the same construct as the FCS creative-domain spine — mapping is methodological-frame, not classification-equivalence; (3) Tier 2 (Phase 5d) **transcribed by visual read** of page-32/34/36 PNG renders because the No. 59 PDF embeds these tables as images — markitdown / pdfplumber / pymupdf all failed; numerical gap values per cell on Table 6.4a were not transcribed (the qualitative finding — SOC group 27 dominance — is the value); (4) Path A (full 770-880-row OECD per-occupation CSV from oecd.ai) remains pending — JS-rendered, blocked from sandbox fetch; SDMX REST probe 2026-06-05 confirmed not yet on the API; DB-updater re-probes quarterly; (5) AI capability scoring updates annually — schema rotates when No. 59's successor drops. **Cross-lens role:** with Stanford HAI's Foundation Model Transparency Index (58 → 40 between editions) and `atana.anthropic_eei` (revealed Claude.ai use), forms a three-corner methodological frame for Atana Index Vol. 2 — **exposure × openness × transparency**.

ETLs: `etl/oecd_ai__papers_headline_to_parquet.py` · `etl/oecd_ai__ai_capability_domains_to_parquet.py` · `etl/oecd_ai__skill_demand_to_parquet.py` · `etl/oecd_ai__occupations_creativity_distance_to_parquet.py` · `etl/oecd_ai__exposure_extremes_ranking_to_parquet.py` · `etl/oecd_ai__felten_oecd_correlations_to_parquet.py` · Methodology: `docs/methodology/oecd_ai_papers.md` · Scoping for the still-pending Path A: `_atana_intel/scoping_oecd_no59_occupations_2026-06-26.md`
...
| **`ciclo1_avaliacao_headlines`** *(2026-07-05)* | 12 | SNIIC seminar 30 Jun–1 Jul 2026 — 15 headline metrics × 6 dimensions. **First beneficiary/agente dimension in the schema:** 167.817 agentes assistidos (145.235 municipal + 22.582 estadual), 58 % interior, 40 % ≤20k hab, execução 95,8 % nacional. Cross-source validates `execucao_financeira` byte-for-byte (R$ 3 bi mobilizado) with a 1,8 pp Δ on execução (late-2025 payments). ⚠️ Tier 1 aggregate — underlying agent-level microdata not yet public; Curious Scientist has watchpoints. |
| **`beneficiarios`** *(Phase 10a, 2026-08-31 — built locally, ⏳ sync pending)* | 167.817 | **Beneficiary microdata — the 167.817-agente file the 2026-07-05 entry reserved.** One row per beneficiário×ente, Ciclo 1, R$ 2,87 bi; document MASKED (LGPD; 64.130 distinct). 59 source cols (provenance suffix `_bbagil`/`_receita_cpf`/`_receita_cnpj`/`_rais`/`_inss`/`_cnefe`/`_cadunico`/`_ibge`) + typed `valor_brl`. Reproduces the seminar identity **145.235 municipal + 22.582 estadual** exactly. Unlocks sex/race/age/CBO/MEI/poverty cuts of the direct transfer — PJ 56 % vs PF 44 %, M R$ 680 mi vs F R$ 582 mi, 78,8 % no RAIS link, geographic inversion vs Rouanet (Note #31). Key `cod_ibge_bbagil` ↔ ente tables. |

⚠️ BRL nominal (not deflated — methodology §4); key is **`cod_ibge`** (no CNPJ in execução/extrato); extrato débitos R$ 6,25 bi ≠ gasto R$ 2,82 bi (inclui aplicações automáticas); Rondônia-estado outlier 0,07 % execução kept; Ciclo 2 = plans only (execution unpublished). `beneficiarios` demographic joins cover only the formal subset (majority NULL) and the document is masked (rows ≠ persons); DF outlier R$ 104.760/beneficiário. `canonical.domain_crosswalk` 91 → **92 rows** (1 `pnab` row → transversal "Multiple — not separable"); coverage 13/14 unchanged. Pairs with `atana.tcu` (Acórdão 1709/2025 audited these cycles).

ETLs: `etl/pnab__to_parquet.py` (Ciclo 1 + Ciclo 2 core; idempotent, byte-identical reruns; 4 source xlsx gitignored under `raw/pnab/_source/`) · `etl/pnab__ciclo1_avaliacao_headlines_to_parquet.py` (SNIIC seminar headline stats; inline data) · `etl/pnab__beneficiarios_to_parquet.py` (Phase 10a microdata; idempotent, byte-identical; 125 MB CSV gitignored under `raw/pnab/_source/`). Methodology: `docs/methodology/pnab_aldir_blanc.md` · `docs/methodology/pnab_beneficiarios.md`
...
| Table | Rows | Description |
|---|---:|---|
| `adesao_entes` | 5.596 | Estados ∪ municípios with `tipo_ente` flag; situação do plano + valor disponível |
| `execucao_financeira` | 10.984 | Per-ente × per-meta execution — **`Meta do Plano` splits AV R$ 2,80 bi from Outras Áreas R$ 1,07 bi**; total R$ 3,86 bi reconciles to law |
| `extratos_bancarios` | 10.967 | Bank-account-level extracts per `Código Plano de Ação` |
| `relatorio_gestao` | 18.180 | **Per-edital narrative + execução-física %** — rich per-action granularity PNAB does not expose |

⚠️ BRL nominal (not deflated — methodology §4); estados execute 105 % (interest income reinvested), munis 98 %; **corrects 2026-06-13 scoping memo's wrong hypothesis** about a separate ANCINE/FSA channel — all R$ 3,86 bi flow through the same MinC pipe, split only by the `meta_plano` flag. Key = `cod_ibge` (uniform across both universes); `adesao_entes.cod_ibge` NULL for munis (use `execucao_financeira` for muni IBGE codes). `canonical.domain_crosswalk` 92 → **93 rows** (1 `lpg` row → *Audiovisual* cultural-focused, "good" confidence); coverage 13/14 unchanged. Pairs with `atana.pnab` on `cod_ibge` (same-ente paired sample) and with `atana.salic` on UF+ano (LPG × Rouanet AV crowding-out — caso 5.2 da A8 Opção B).

ETL: `etl/lpg__to_parquet.py` (idempotent, byte-identical reruns; 4 CSV + 2 XLSX gitignored under `raw/lpg/_source/`) · Methodology: `docs/methodology/lpg_paulo_gustavo.md`

---

## `atana.anthropic_eei` — Anthropic Economic Index (revealed-usage frame) ⏳ Phase 6b.2 refresh built locally, pending push (Tier-1 tables still Live from 2026-06-14 at release_2026_03_24 vintage)
...
|---|---:|---|---|
| `country_usage` | 178 | Feb 2026 week | Country share of global Claude.ai usage — **Brazil 2.55 %** |
| `task_usage_by_country` | 5,321 | Feb 2026 week | O*NET-task usage for GLOBAL/US + the 5 corpus countries — **Brazil-native rows (269 tasks)** |
| `collaboration_by_country` | 42 | Feb 2026 week | Automation-vs-augmentation collaboration patterns by geo — BR more directive (35.0 %), less learning (16.7 %) than global |
| `occupation_usage_global_v2` | 749 | global v2 (early 2025) | **DERIVED** — SOC-occupation usage shares (equal apportionment across statement-sharing occupations); **SOC 27-\* cultural occupations = 9.4 %** of matched usage |

⚠️ Usage ≠ exposure ≠ automation risk; selected population; vintages differ by table; full occupation-level OECD pairing gated on `oecd_ai` Tier 2. The 103 MB raw geography file is cached gitignored; only derived tables are committed. `canonical.domain_crosswalk` 90 → **91 rows**; coverage 13/14 unchanged.

ETL: `etl/anthropic_eei__to_parquet.py` (HF fetch + cache, `--refresh` = DB-updater job) · Methodology: `docs/methodology/anthropic_eei.md`

---

## `atana.ilo_ai` — ILO refined GenAI occupational-exposure index (expert-exposure frame) ⏳ Phase 6c built locally, pending push + MotherDuck sync

Source: **ILO (2025), refined global index of GenAI occupational exposure** (Gmyrek, Berg et al.; "How might generative AI impact different occupations", ilo.org). The **fourth** AI lens — ISCO-coded expert *exposure* with a task-variability dimension, distinct from `oecd_ai` (SOC capability) and `anthropic_eei` (SOC revealed use). Two published CSVs, founder-downloaded 2026-07.

| Table | Rows | Description |
|---|---:|---|
| `occupation_exposure` | 427 | Exposure score (0.09–0.70) + 6-level gradient per ISCO detailed occupation; Atana cultural cut = 26 occupations (`is_cultural_creative` / `cultural_tier`) |
| `exposure_distribution` | 60 | % of employment by World / 4 income groups / 5 regions × 6 gradients (long) |

**The finding:** the index reproduces the Authenticity-Paradox split from a third expert methodology — writing/language creatives high (gradient 3: Translators 0.59, Authors 0.55, Journalists 0.54), performing/visual/craft artists Not Exposed (Musicians 0.28, Visual Artists 0.21, Dancers 0.19). Exposed creatives sit in the *high-task-variability* (augmentation) zone, not the gradient-4 full-automation zone. Four frames — OECD demand, EEI revealed use, RAIS wages, ILO exposure — converge on one split. Macro: exposure is a rich-country phenomenon (top-two gradients: High income 17.1 %, World 7.5 %, Americas 11.5 %, Low income 1.1 %).

⚠️ Exposure ≠ automation ≠ displacement; task variability is the augmentation flag; ISCO join to the corpus (CBO/SOC) needs a crosswalk (deferred 6b.2); the cultural cut is an Atana editorial flag over ISCO titles, reproducible in the ETL. `canonical.domain_crosswalk` 95 → **96 rows** (1 `ilo_ai` row → Intellectual property, ★ frame); coverage 13/14 unchanged.

ETL: `etl/ilo_ai__exposure_to_parquet.py` (idempotent; `_source/` CSVs cached) · Methodology: `docs/methodology/ilo_ai.md`
...
## `atana.cl_csc` — Chile Cuenta Satélite de Cultura (BCCh + MINCAP + CEPAL) ⏳ Built locally (pending push + MotherDuck sync)

The **fifth non-Brazilian national source** in the corpus, after Mexico (`atana.inegi`), Colombia (`atana.dane`), Argentina (`atana.sinca`) and Costa Rica (`atana.cr_bccr`). Phase 7a.0 — Tier 1 ingest of the **first results** of Chile's CSC pilot (launched 2024), as quoted verbatim in MINCAP's *Informe Final Cuenta Pública Participativa 2025*.

| Table | Rows | Vintage | Description |
|---|---:|---|---|
| `bcch_pilot_headline_consumption` | 3 | 2018, ≈2020, 2022 | Household effective cultural consumption, % share of total household effective consumption. 2018=1.7 %, pandemic-low=1.4 %, 2022=1.4 %. The single headline indicator publicly stated at this stage of the pilot. |

⚠️ Maturity caveat: this is a **3-row Tier 1 ingest, not a full CSC**. The pilot has published headline indicators only; the underlying tables (production, employment, ACC × VA) are not yet a continuous open series. The deeper Chilean data lives in **INE Chile *Estadísticas Culturales: Informe Anual 2024*** — that's the **Phase 7a.1** target (deferred to a dedicated session, ~4-6 days ETL effort).

The intermediate years 2019/2021 are NOT in the table — MINCAP's prose does not state them and inferring them would assert structure the source does not. The 2020 row carries the soft label `≈2020 (pandemic low)` because the source writes "hasta la pandemia" rather than nominating a specific year.

Chile is the **first LATAM country to enter via a non-trade lens** — direct consequence of pilot maturity. `canonical.domain_crosswalk` extended **93 → 94 rows** (1 new `cl_csc` row → FCS *Social participation* transversal domain, ★ approximate proxy mapping, same logic as the IBGE TIC/turismo rows). FCS coverage meter **13/14 unchanged** — Chile is a new source in an already-reached domain.

ETL: `etl/cl_csc__bcch_pilot_headline_to_parquet.py` (inline data → DuckDB COPY → Parquet, idempotent, byte-identical reruns; `ATANA_ETL_SKIP_PUSH` guard) · Methodology: `docs/methodology/cl_csc.md` · Scoping: `_atana_intel/phase7_chile_uruguay_scoping.md`
...
The Atana harmonisation crosswalk — maps every cultural-statistics classification in the corpus onto one common spine. **96 rows**, one per classification code (Phase 3 built 72; Phase 4 added 10 `ibge_siic` rows and the `bcb` / `inpi` / `ecad` rows; Phase 5 added `cisac` / `ifpi` and `luminate` / `tcu` / `oecd_ai`; Phase 6 added `anthropic_eei` and `ilo_ai` — the third and fourth AI lenses; Phases PNAB + LPG added the two Brazilian cultural-governance direct-transfer lines; Phase 7a.0 added `cl_csc`; Phase 11 Tier 1 added `emendas` — the fourth federal cultural-funding pipe).

| Column | Type | Description |
|---|---|---|
| `source_schema` | VARCHAR | `fcs2025` / `inegi` / `dane` / `sinca` / `cr_bccr` / `unctad` / `ibge_comex` / `ibge_siic` / `bcb` / `inpi` / `ecad` / `cisac` / `ifpi` / `luminate` / `tcu` / `oecd_ai` / `anthropic_eei` / `ilo_ai` / `pnab` / `lpg` / `cl_csc` / `emendas` |
...
ETL: `etl/canonical__build_latam_trade_by_fcs.py` · Methodology: `docs/methodology/latam_trade_by_fcs_domain.md`

### `canonical.cultural_sector_baseline_a27` ⏳ Built locally (pending push + MotherDuck sync)

The Análise 27 reference baseline for the Brazilian cultural sector as a curated layer — **2 tables, 273 rows**, grain `recorte × indicator × year` (trajectory) and `ruler` (three rulers snapshot 2023).

| Table | Rows | What it holds |
|---|---:|---|
| `cultural_sector_baseline_a27_trajectory` | 270 | 9 recortes × 5 indicators × 6 years (2013, 2019–2023); both nominal and IPCA-deflated to BRL 2023 |
| `cultural_sector_baseline_a27_three_rulers` | 3 | IBGE SIIC inteiro (R$ 387,9 bi) · IBGE só centrais (R$ 138,5 bi) · UNESCO FCS 2025 conservative (R$ 157,0 bi, range 150–164) |

Built from `atana.ibge_estruturais.tab_2_1`–`tab_2_8` with the IPCA deflator from `atana.macro.ipca`; the FCS row uses `canonical.domain_crosswalk` and the share assumptions of Análise 27 §11. **Includes a critical correction:** v1.0/v1.1 of Análise 27 captured only one of the two telecom CNAEs in the SIIC cultural-periphery via openpyxl substring matching (R$ 18,8 bi); this curated layer aggregates both ("Telecomunicações por fio…" R$ 87,3 bi + "Outras atividades de telecomunicações" R$ 18,8 bi = R$ 106,1 bi), exposed via `n_source_rows_aggregated`. Substantive implication: telecom was flat-to-declining in real terms (−1.2 %), not +53 % as v1.1 claimed; the story of real growth is software alone (+76 %).

**Consumers:** Análises 14/25, Note #21, Vol. 1 of the book (reference chapter), Vol. 2 of the Atana Index (Brazilian-baseline anchor).

ETL: `etl/canonical__build_cultural_baseline_a27.py` (inline IPCA constants → DuckDB COPY → Parquet, idempotent, byte-identical reruns) · Methodology: `docs/methodology/canonical_cultural_baseline_a27.md`
...
| 2026-08-31 | **Phase 10a — `atana.pnab.beneficiarios` added (NEW table)** — the 167.817-agente beneficiary microdata the 2026-07-05 entry reserved has dropped. Source: MinC Portal de Dados da Cultura *"PNAB dados abertos completo"* (BB-Ágil × Receita CPF/CNPJ × RAIS × INSS × CNEFE × CadÚnico × BPC × IBGE), CC BY 4.0. **167.817 rows × 59 cols + `valor_brl`, R$ 2,87 bi**; document masked (LGPD; 64.130 distinct; rows ≠ persons). Reproduces the seminar identity **145.235 municipal + 22.582 estadual = 167.817** exactly, and reconciles to `execucao_financeira` Ciclo 1. Unlocks the demographic cuts the aggregates cannot: PJ 56 % vs PF 44 %, sex M R$ 680 mi vs F R$ 582 mi, 78,8 % no RAIS link / 3,4 % MEI, geographic inversion vs Rouanet (SP+RJ 60 % Rouanet vs 26 % PNAB; HHI 2.256 vs 748) — consumed by **Note #31** "Dois instrumentos, alcances opostos". `canonical.domain_crosswalk` unchanged (inherits the `pnab` row; cultural dimension via embedded CNAE/CBO flags); coverage 13/14 unchanged. ETL `etl/pnab__beneficiarios_to_parquet.py` (idempotent, byte-identical md5 `a1653b14…`; 125 MB CSV gitignored). Validations 6/6 (rows, R$ total, PJ>PF, M>F, NE>SE beneficiaries, municipal/estadual identity). Methodology `docs/methodology/pnab_beneficiarios.md`. **Built locally — pending GitHub push + first MotherDuck sync (João).** |
| 2026-07-05 | **Phase 10 Move A — `atana.pnab.ciclo1_avaliacao_headlines` added (NEW table)** — first beneficiary/agente dimension in the schema. Source: MinC/SNIIC studies presented at the *I Seminário de Avaliação de Resultados da PNAB* (30 Jun–1 Jul 2026), aggregate-headline Tier 1 (agent-level microdata not yet public). 12 rows × 6 analytical dimensions (beneficiário/financeiro/execução/territorial/cobertura/metodológico). Load-bearing findings: **167.817 agentes assistidos** (145.235 municipal + 22.582 estadual — identity validated), **58 % interior + 40 % ≤20k hab** (direct backing for the Note #21 territorial argument), **execução 95,8 % nacional** (states 97.1 %, munis 94.4 %), R$ 3 bi mobilizado (cross-source validates `atana.pnab.execucao_financeira` byte-for-byte), R$ 800 mi em ações afirmativas (first quantification). Curious Scientist has 3 watchpoints active for microdata release (see `_atana_intel/agents/curious_scientist.md`); Phase 10a will consume when 167,817-row microdata drops. ETL `etl/pnab__ciclo1_avaliacao_headlines_to_parquet.py` (inline data, idempotent, byte-identical reruns). **Built locally — pending push + MotherDuck sync (João).** |
| 2026-07-05 | **Phase 9a — `atana.salic` coverage-state audit + doc sync (documentation only, no ingest).** DB-updater's proposed 6-hour SALIC refresh was cancelled after audit revealed the corpus's `projetos` table has been at **48,189 rows since commit `4e48176` (2026-06-13)** — not 26,203 as the docs, manifest, /data/ page, CLAUDE.md, and Análise 7 all cited. The 26,203 figure was the count of `projetos_v2`, a legacy filtered subset. MotherDuck already unified both tables at 48,189 in the 2026-06-14 sync pass; only GitHub's `projetos_v2.parquet` still holds the 26k legacy snapshot as a compat artifact. Under a "promote projetos to canonical" decision (2026-07-05), the following changed: (a) `docs/methodology/salic_api.md` — §1 headline reframe, §1.1 coverage-state clarification added, table 2 updated to show `projetos` as canonical at 48,189 with year distribution 2019-2026, W1 rewritten to 85 % coverage, W8 added for the missing local `edges_pronac_incent.parquet`; (b) `docs/manifest.md` `atana.salic` section rewritten with the correct table row counts; (c) `atana_site/data.yaml` SALIC card bumped (Phase 9a wrap-up); (d) `_atana_intel/db_update_log.md` entry with the full audit trail. Coverage claim goes from ~46 % → **~85 % of the 2019-2026 Rouanet universe**. **No new ingest, no MotherDuck touch, no Bright Data credits burned.** Published Notes/Análises citing the earlier 26,203/46 % remain correct at time of writing; new work uses 48,189. **Built locally — pending push (João).** |
| 2026-07-30 | **Phase 6c — `atana.ilo_ai` schema added (NEW)** — the **fourth AI-exposure lens** in the corpus, and the first ISCO-coded one. Source: ILO (2025) refined global index of GenAI occupational exposure (Gmyrek, Berg et al.), two founder-downloaded CSVs. 2 tables, 487 rows: `occupation_exposure` (427 ISCO occupations × 0.09–0.70 score + 6-level exposure/task-variability gradient; Atana editorial cultural cut = 26 occupations) + `exposure_distribution` (60; % of employment by World / income group / region × 6 gradients). **The finding:** the index reproduces the Authenticity-Paradox split from a third *expert* methodology — writing/language creatives high (gradient 3: Translators 0.59, Authors 0.55, Journalists 0.54), performing/visual/craft artists Not Exposed (Musicians 0.28, Visual Artists 0.21, Dancers 0.19); exposed creatives sit in the high-task-variability (augmentation) zone, not gradient 4 (full automation). Four frames — OECD demand, EEI revealed use, RAIS wages, ILO exposure — now converge on one split. `canonical.domain_crosswalk` 95 → **96 rows** (1 `ilo_ai` row → Intellectual property, ★ frame); coverage 13/14 unchanged. ETL `etl/ilo_ai__exposure_to_parquet.py` (idempotent, byte-identical reruns confirmed); methodology `docs/methodology/ilo_ai.md`; consumed by Análise 32 (Brazil-workforce-weighted exposure). ⚠️ Exposure ≠ automation ≠ displacement; ISCO join to the corpus (CBO/SOC) needs a crosswalk (deferred 6b.2); the cultural cut is an Atana editorial flag over ISCO titles. **Built locally — pending push + first MotherDuck sync (João).** |
| 2026-06-30 | **Phase 6b.2 — `atana.anthropic_eei` refreshed to release_2026_06_26** ("Cadences" report). DB-updater flagged the new release; verification found the June schema is a full rewrite, not a drop-in. **Path B (in-place refresh, corpus schema preserved) executed:** ETL rewritten for the new long-format schema (`category_name` / `hierarchy_level` / `metric_id` / `node_name` / `value`), ISO-3 → ISO-2 mapping added for the 7 corpus geos, `usage_count` / `task_count` / `collaboration_count` populated as NULL (Anthropic dropped raw counts), May 2026 monthly aggregate as primary vintage (`date_end='2026-06-01'`). All 4 corpus tables preserve their column shapes; downstream analyses (Note #18) continue to work. Six migrations documented in `docs/methodology/anthropic_eei.md` §1.1. New optional dimensions (subregion — all 24 major BR UFs now published, artifact metrics, per-capita usage index, 6 collaboration pattern splits) deferred to Phase 6b.3. **Built locally — pending push + MotherDuck sync (João).** |
| 2026-06-26 | **Phase 5d — `atana.oecd_ai` Tier 2 extended (NEW tables)** — three PDF-extracted occupation-level tables from OECD No. 59 chapter 6 added to the existing methodological-frame schema. **+40 rows across 3 tables:** `occupations_creativity_distance` (10 — Table 6.4a Creativity column, 8 of 10 SOC group 27 cultural, the occupation-weighted Authenticity Paradox anchor), `exposure_extremes_ranking` (20 — Table 6.5 top/bottom 10 overall, 0 SOC-27 in either tail), `felten_oecd_correlations` (10 — Table 6.6 per-domain, **overall correlation only 0.34, Creativity outlier 0.25**, the empirical proof Felten is creativity-blind). Path B of the scoping memo — Path A (the JS-rendered oecd.ai per-occupation CSV) remains blocked from sandbox fetch; DB-updater re-probes SDMX quarterly. Schema now at 6 tables / 57 rows. `canonical.domain_crosswalk` unchanged at 94 rows (the existing `oecd_ai` row's `notes` field was refreshed to enumerate the new tables — same convention as `atana.tcu` / `atana.ifpi` keeping a single crosswalk row per schema). 3 new ETLs, all idempotent (byte-identical reruns confirmed). Methodology doc `docs/methodology/oecd_ai_papers.md` extended with three new table subsections. **Built locally — pending push + MotherDuck sync of the 3 new tables (João).** |
| 2026-06-16 | **Phase 7a.0 — `atana.cl_csc` schema added (NEW)** — Chile is the fifth non-Brazilian national source after MX/CO/AR/CR. Tier 1 ingest of the BCCh+MINCAP+CEPAL CSC pilot first results (launched 2024), via verbatim prose in MINCAP's *Informe Final Cuenta Pública Participativa 2025*. 1 table, **3 rows** — `bcch_pilot_headline_consumption` carries the household effective cultural consumption share for 2018 (1.7 %), pandemic-low ≈2020 (1.4 %) and 2022 (1.4 %). Intermediate years 2019/2021 NOT included (not stated by source). `canonical.domain_crosswalk` extended **93 → 94 rows** (1 new `cl_csc` row → *Social participation* transversal, approximate proxy mapping with ★ flag, same logic as IBGE TIC/turismo). Coverage 13/14 unchanged. ETL `etl/cl_csc__bcch_pilot_headline_to_parquet.py` (idempotent, byte-identical reruns confirmed); methodology `docs/methodology/cl_csc.md`; scoping `_atana_intel/phase7_chile_uruguay_scoping.md`. **Phase 7a.1** (deeper INE *Estadísticas Culturales Informe Anual 2024* — 3-table employment + production + participation ingest) deferred to a dedicated session. **Phase 7b** (Uruguay) deliberately not pursued — confirmed stalled at 2012 data; documented in the scoping memo. **Built locally — pending push + first MotherDuck sync of `atana.cl_csc` (João).** |
| 2026-06-16 | **Phase A27 — `canonical.cultural_sector_baseline_a27` materialised (NEW)** — the Análise 27 reference baseline as a curated layer. 2 tables, 273 rows: `cultural_sector_baseline_a27_trajectory` (270 rows; 9 recortes × 5 indicators × 6 years, nominal + IPCA-deflated to BRL 2023) + `cultural_sector_baseline_a27_three_rulers` (3 rows; IBGE inteiro R$ 387,9 bi · IBGE centrais R$ 138,5 bi · UNESCO FCS 2025 R$ 157,0 bi, range 150–164). Sourced from `atana.ibge_estruturais` × `atana.macro.ipca`; FCS row reclassified through `canonical.domain_crosswalk` with §11 share assumptions. **Critical correction at ingest:** v1.0/v1.1 of Análise 27 captured only one of two telecom CNAEs in the SIIC cultural-periphery (R$ 18,8 bi) via openpyxl substring matching; this layer correctly aggregates both into R$ 106,1 bi total (`n_source_rows_aggregated = 2` for the `telecom` recorte). The Análise was patched to v1.2 in the same session — telecom story shifts from "+53 % real" to "−1.2 % real" (flat-to-declining), making software (+76 % real) the sole driver of real growth in Brazilian cultural VA. Crosswalk unchanged at 93 rows (the curated layer consumes classifications, doesn't introduce them — same convention as `canonical.cmo_directory_alcam` and `canonical.latam_trade_by_fcs_domain`). ETL `etl/canonical__build_cultural_baseline_a27.py` (idempotent, byte-identical reruns; tempfile-based writes for sandbox cross-mount safety); methodology `docs/methodology/canonical_cultural_baseline_a27.md`. **Built locally — pending push + first MotherDuck sync (João).** |
| 2026-06-14 | **OG-schema documentation gap closed — 6 new methodology notes + audit recursion.** Six original Phase-1/Phase-2 schemas (`ibge_pnadc`, `ibge_comex`, `lexml`, `rais`, `salic`, `unctad`) lacked methodology notes under `docs/methodology/`; they had reference material in other locations (skills, `docs/rais_methodology.md`, ETL comments) but did not carry the Status-block convention. New notes added: `ibge_pnadc_siic_ch6.md`, `ibge_comex_siic_ch10.md`, `lexml_genealogy.md`, `rais_mte.md` (references the existing `docs/rais_methodology.md` rather than duplicating), `salic_api.md`, `unctad_creative_economy.md`. Each carries a one-line Status block + provenance + tables + caveats. `etl/_audit_methodology_status.py` extended: (a) `NOTE_TO_SCHEMA` now maps the 6 new notes; (b) `find_parquet_paths` uses recursive `rglob` (skipping `_source` / `_staging`) — RAIS's Hive-style `year=YYYY/part-0.parquet` layout previously returned 0 tables; (c) `count_rows` is now resilient to corrupt files, returning a `n_bad` counter that surfaces in the Status line. Corpus state: methodology-note coverage **30 / 27 schemas** documented (3 canonical tables get their own notes + 2 ecad notes for v3 versions). **One corrupt parquet flagged** — `raw/unctad/goods_growth.parquet` (33 MB, no magic bytes since May 16); the per-year `goods_value_*` tables remain healthy and the corruption only affects the unused growth aggregation. **Built locally — pending push (João).** |
