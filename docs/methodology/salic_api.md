...
`atana.salic` is a **bulk-downloaded corpus of every PRONAC project, every proponent, every incentivador (donor), and the per-project execution graph** — **58,532 projects, 2019–2026, ~98 % of the current API universe (59,642)** (canonical table `projetos`). The 2019–2022 window is stable; the most recent refresh (**12/08/2026**, targeted 2023–2026 year-slice, **+10,343 records** after the ~5-week API blackout) re-pulled through the FlareSolverr WAF bypass. The residual ~1,110 vs the 59,642 headline are pre-2019 / year-null projects outside the 2019–2026 discovery window (scope, not lag). Coverage of the 2019–2026 per-year universe is **100 %**.
...
**Canonical citation going forward:** 58,532 projects, ~98 % of the current API universe (100 % of the 2019–2026 per-year universe). *(Was 48,189 / ~85 % until the 2026-08-12 year-slice refresh; and 26,203 / 46 % before the 2026-07-05 coverage-state audit — that older figure was the legacy `projetos_v2` subset.)* `projetos_v2` is documented as a legacy compat table; new analyses should query `projetos`. Published Notes and Análises (as of publication date) that cite the earlier figures are correct at time of writing — no retroactive rewrite; new work uses the fuller state.

**Year distribution of the 58,532** (after 2026-08-12 refresh): 2019: 3,150 · 2020: 4,394 · 2021: 2,320 · 2022: 2,346 · 2023: 10,722 · 2024: 14,218 · 2025: 15,415 · 2026 (partial): 5,967. *(Prior 48,189 vintage — 2023: 9,436 · 2024: 10,388 · 2025: 13,197 · 2026: 2,958.)* The 2019–2022 years were untouched by the year-slice; 2023–2026 gained +1,286 / +3,830 / +2,218 / +3,009.

## 2. Tables (6 tables)

| Table | Rows | What |
|---|---:|---|
| **`projetos`** *(canonical)* | **58,532** | The canonical project table — one row per PRONAC. Carries the headline financials (`aprovado`, `captado`, `taxa_captacao`), classification (`area`, `segmento`, `mecanismo` Art. 18 vs Art. 26), geography (`uf_proponente`, `municipio_proponente`), and life-cycle (`situacao`, `data_termino`, `data_inicio`, `valor_aprovado`, `valor_captado`, `local_realizacao`). ~98 % of the live API universe; 2019–2026 window complete (2026-08-12 refresh). |
...
- **Refresh cadence:** quarterly recommended. Current vintage: **2026-08-12 targeted year-slice** (`salic__bulk_download.py --years 2023,2024,2025,2026`; +10,343 records; 48,189 → 58,532) after the ~5-week API blackout (Jul–Aug 2026, WAF tightening). Prior vintage 2026-06-13 (`4e48176`). Runbook: `etl/SALIC_YEAR_SLICE_REFRESH_RUNBOOK.md`; delta reconcile: `_atana_intel/phase9b_salic_delta_reconcile.py`; the WAF bypass needs a local FlareSolverr container (`FLARESOLVERR_URL=http://localhost:8191/v1`).
