...
Strategy:
- Small tables (services_countries, services_regional): single Parquet file
- Large tables: goods_value (25M rows) partitioned by year; goods_growth (24M rows)
  partitioned by period — one file per partition, each well under GitHub's 100 MB
  per-file limit. (goods_growth as a single file is ~115 MB and GitHub rejects it.)

Each year of goods_value compresses to ~13 MB with ZSTD; each period of goods_growth
to ~4-5 MB — both well under GitHub's 100 MB per-file limit.
...
def export_partitioned_by_period(table: str, out_dir: Path) -> None:
    # goods_growth keys on Period (e.g. 20022003), not Year; one file per period
    # keeps each well under GitHub's 100 MB limit (the single file is ~115 MB).
    out_dir.mkdir(parents=True, exist_ok=True)
    periods = con.execute(
        f"SELECT DISTINCT Period FROM atana.unctad.{table} ORDER BY Period"
    ).fetchall()
    for (p_,) in periods:
        p = out_dir / f"{table}_{p_}.parquet"
        if p.exists() and p.stat().st_size > 1000:
            continue
        t0 = time.time()
        con.execute(
            f"COPY (SELECT * FROM atana.unctad.{table} WHERE Period={p_}) "
            f"TO '{p}' (FORMAT PARQUET, COMPRESSION ZSTD)"
        )
        size_mb = p.stat().st_size / 1024 / 1024
        print(f"  ✓ {p.name} — {size_mb:.2f} MB ({time.time() - t0:.1f}s)")
...
def main() -> None:
    ...
    print()
    print("Large tables (partitioned, one file per partition):")
    export_partitioned_by_year("goods_value", OUT / "goods_value")
    export_partitioned_by_period("goods_growth", OUT / "goods_growth")
